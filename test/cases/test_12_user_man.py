...
def test_user1_update(server):
    c0 = get_conn('u0')
    c0.update_user('u1', admin=True, max_storage='2G', permission=FileReadPermission.PROTECTED)
    ...
    c0.update_user('u1', admin=False, permission='public')
    u1_info = c0.query_user('u1')
...
def test_set_peer_access(server):
    ...
    assert c0.list_peers(as_user='u2', admin=False)[0].username == 'u3', "Peer listing failed"
    assert c0.list_peers(as_user='u3', incoming=True, admin=False)[0].username == 'u2', "Peer listing failed"